---
id: "THEME_JUAREZ_ZAPOTEC_IDENTITY_LIMITS"
type: "theme"
status: "stub"
created: "2026-06-22"
updated: "2026-06-22"
tags:
  - "theme"
  - "hamnett"
  - "juarez"
source_id: "SRC_HAMNETT_1994_JUAREZ"
theme_kind: "identity_and_politics"
verification_needed: "yes"
---

# Juárez Zapotec Identity Limits

## Definition

JuárezのZapotec出自と、IsthmusのZapotec共同体との政治的・文化的同一化を区別するための主題。

## Historical Role

Hamnett p.41では、JuárezがZapotecであっても、IsthmusのZapotecとは文化・言語集団が異なり、自動的な同一化関係はなかったと整理される。

## Creative Use

Juárezを「先住民出身の国民的英雄」とだけ描くことへの留保になる。

## Open Questions

- Hamnettの解釈か、具体的史料に基づく事実整理かを確認する。

## Linked Items

- People: [[Benito_Juarez]]
- Organizations: [[Indigenous_Communities_Of_The_Isthmus]]
- Themes: [[Zapotec_Identity]], [[Zapotec_Communities]], [[Indigenous_Community_Autonomy]]
- Fact Cards: [[FACT_HAMNETT_JUAREZ_0465]]
- Source Notes: [[CAP_HAMNETT_JUAREZ_0023]]

## Hamnett 1994 p.44-45 Notes

p.44-45では、JuárezがHispanic city of Oaxacaの世界観を身につけ、先住民利害を自由主義の観点から見たとHamnettが評価する。Loxichaへの働きかけは先住民人口を拒絶していなかった証左として扱われる。

- Capture: [[CAP_HAMNETT_JUAREZ_0025]]
- Related Fact Cards: [[FACT_HAMNETT_JUAREZ_0507]], [[FACT_HAMNETT_JUAREZ_0517]], [[FACT_HAMNETT_JUAREZ_0518]], [[FACT_HAMNETT_JUAREZ_0519]]
- Related Themes: [[Liberalism_Through_Hispanic_Urban_Worldview]], [[Indigenous_Community_Autonomy]]
- Cautions: Juárezを単純な先住民代表として描かない。

## Hamnett 1994 pp.236–237 Conclusion Note

Hamnett explicitly distinguishes Juárez's Zapotec origin from representation of a specific Indigenous revindication movement or Oaxaca peasant culture and places his political self-identification in mid-century republican Liberalism.

- Capture: [[CAP_HAMNETT_JUAREZ_0121]]
- Related Fact Card: [[FACT_HAMNETT_JUAREZ_2916]]
- Cautions: `Indian revindication` remains a translation and concept-verification question.
