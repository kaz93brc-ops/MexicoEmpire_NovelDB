---
id: "THEME_FUERO_ECLESIASTICO"
type: "theme"
status: "stub"
created: "2026-07-01"
updated: "2026-07-01"
tags:
  - "theme"
  - "hamnett"
  - "church-state"
source_id: "SRC_HAMNETT_1994_JUAREZ"
canonical_name: "fuero eclesiástico"
aliases:
  - "fuero eclesiástico"
  - "ecclesiastical fuero"
verification_needed: "yes"
---

# Fuero eclesiástico

## Historical Role

Hamnett pp.96-97 treats the ecclesiastical fuero as the jurisdictional privilege contested by the Juárez Law and defended by Munguía as more than a mere privilege.

## Creative Use

Use it as a jurisdictional conflict over courts and sovereignty, not as a generic symbol of faith.

## Open Questions

- Confirm the legal scope before and after the Juárez Law and compare Munguía's claim with the statutory text.

## Linked Items

- Source: [[SRC_HAMNETT_1994_JUAREZ]]
- Capture: [[CAP_HAMNETT_JUAREZ_0051]]
- Related Theme: [[Fueros]]
- Events: [[Juarez_Law_On_Administration_Of_Justice_1855_11_23]]
- People: [[Clemente_de_Jesus_Munguia]], [[Benito_Juarez]]
- Fact Cards: [[FACT_HAMNETT_JUAREZ_1108]], [[FACT_HAMNETT_JUAREZ_1111]]

## Hamnett 1994 Index pp.290–291 Locator

- Capture: [[CAP_HAMNETT_JUAREZ_0148]]; `fuero eclesiástico` appears as a selected subentry under fueros.
- Caution: jurisdictional content, reform, and institutional consequences require narrative and legal-text verification.
