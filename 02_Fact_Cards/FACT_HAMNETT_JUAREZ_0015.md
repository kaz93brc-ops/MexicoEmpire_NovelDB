---
id: "FACT_HAMNETT_JUAREZ_0015"
type: "fact_card"
status: "draft"
created: "2026-06-14"
updated: "2026-07-13"
tags: "fact-card;hamnett;juarez;preface"
source_id: "SRC_HAMNETT_1994_JUAREZ"
source_title: "Juárez"
author: "Brian R. Hamnett"
section: "PREFACE"
printed_page: "xii"
archive_page: "n12 / screenshot view page 13 of 323"
screenshot_file: "スクリーンショット 2026-06-13 17.22.31.jpeg"
printed_page_status: "confirmed"
title: "Juárez自身も自己像の形成に意識的だった"
statement: "Hamnettは、Juárez自身がイメージの力を意識し、服装を含めた独自の政治的人格を作り上げたと述べている。"
evidence_category: "author_interpretation"
confidence: "confirmed"
verification_needed: "yes"
source_note: "Juárezの自己演出への言及。"
japanese_note: "Juárezの服装・態度・孤立した姿勢を政治的演出として扱える。"
creative_use: "大統領の沈黙、黒服、簡素さ、儀礼的姿を象徴場面に利用可能。"
cautions: "具体的な服装描写はこのページだけでは詳細不明。"
people:
  - "Benito Juárez"
events:
  - "Juárez public image formation"
places:
  - "Mexico"
organizations:
  - "Republic of Mexico"
themes:
  - "political image-making"
  - "national sovereignty"
  - "republican virtue"
related_capture: "CAP_HAMNETT_JUAREZ_0002"
additional_related_captures:
  - "CAP_HAMNETT_JUAREZ_0121"
---

# Juárez自身も自己像の形成に意識的だった

## Claim

Hamnettは、Juárez自身がイメージの力を意識し、服装を含めた独自の政治的人格を作り上げたと述べている。

## Evidence / Citation

- Category: author_interpretation
- Confidence: confirmed
- Verification needed: yes
- Source: [[SRC_HAMNETT_1994_JUAREZ]]
- Source title: Juárez
- Author: Brian R. Hamnett
- Section: PREFACE
- Printed page: xii
- Archive page: n12 / screenshot view page 13 of 323
- Screenshot file: スクリーンショット 2026-06-13 17.22.31.jpeg

## Notes

- Source note: Juárezの自己演出への言及。
- Japanese note: Juárezの服装・態度・孤立した姿勢を政治的演出として扱える。
- Cautions: 具体的な服装描写はこのページだけでは詳細不明。
- Creative use: 大統領の沈黙、黒服、簡素さ、儀礼的姿を象徴場面に利用可能。

## Links

- People: [[Benito_Juarez|Benito Juárez]]
- Events: [[Juarez_Public_Image_Formation|Juárez public image formation]]
- Places: [[Mexico]]
- Organizations: [[Republic_of_Mexico|Republic of Mexico]]
- Themes: political image-making, national sovereignty, republican virtue
- Related Capture: [[CAP_HAMNETT_JUAREZ_0002]]

## Conclusion Cross-Reference

- [[CAP_HAMNETT_JUAREZ_0121|Conclusion pp.236–237]] reiterates that Juárez was acutely conscious of political image and that the Juárez myth reached back into his lifetime.
- Planned `FACT_HAMNETT_JUAREZ_2912` was not created because this card already holds the same claim.
