---
id: "FACT_HAMNETT_JUAREZ_0771"
type: "fact_card"
status: "draft"
created: "2026-06-24"
updated: "2026-07-16"
tags: "fact-card;hamnett;juarez;chapter3;lerdo-law"
source_id: "SRC_HAMNETT_1994_JUAREZ"
source_title: "Juárez"
author: "Brian R. Hamnett"
section: "Chapter 3 / JUÁREZ AND LIBERALISM / DEEPENING DIVISIONS IN THE LIBERAL PARTY"
printed_page: "p.65"
archive_page: "/page/64/mode/2up; Page 64 (79/323)"
page: "printed page 65"
kindle_location: ""
screenshot_file: "スクリーンショット 2026-06-13 17.27.50.jpeg"
printed_page_status: "confirmed"
title: "Miguel Lerdo issued the Lerdo Law on 25 June 1856 with congressional support"
statement: "Miguel Lerdoは、1856年6月25日のLerdo Lawを、議会の支持を得て発布した。"
evidence_category: "historical_fact"
confidence: "probable"
verification_needed: "yes"
claim: "Miguel Lerdoは、1856年6月25日のLerdo Lawを、議会の支持を得て発布した。"
source_note: "Lerdo Lawの発布日と議会支持を要約した。"
japanese_note: "正式名称・議会支持の手続は要確認。"
creative_use: "財務大臣Lerdoが改革を法令化する場面に使える。"
cautions: "発布主体と公布文言は注22で確認する。"
related_capture: "CAP_HAMNETT_JUAREZ_0035"
people:
  - "Miguel_Lerdo_De_Tejada"
events:
  - "Lerdo_Law_1856_06_25"
places: []
organizations:
  - "Finance_Ministry"
themes:
  - "Liberal_Reform"
  - "Disentailment_Disamortization"
related_timeline_entries:
  - "TIME_HAMNETT_JUAREZ_0318"
---
# Miguel Lerdo issued the Lerdo Law on 25 June 1856 with congressional support

## Claim

Miguel Lerdoは、1856年6月25日のLerdo Lawを、議会の支持を得て発布した。

## Evidence / Citation

- Category: historical_fact
- Confidence: probable
- Verification needed: yes
- Source: [[SRC_HAMNETT_1994_JUAREZ]]
- Printed page: p.65
- Archive page: /page/64/mode/2up; Page 64 (79/323)
- Screenshot file: スクリーンショット 2026-06-13 17.27.50.jpeg

## Notes

- Source note: Lerdo Lawの発布日と議会支持を要約した。
- Japanese note: 正式名称・議会支持の手続は要確認。
- Cautions: 発布主体と公布文言は注22で確認する。
- Creative use: 財務大臣Lerdoが改革を法令化する場面に使える。

## Links

- People: [[Miguel_Lerdo_De_Tejada]]
- Events: [[Lerdo_Law_1856_06_25]]
- Places:
- Organizations: [[Finance_Ministry]]
- Themes: [[Liberal_Reform]], [[Disentailment_Disamortization]]
- Related Capture: [[CAP_HAMNETT_JUAREZ_0035]]
- Related Timeline Entries: [[TIME_HAMNETT_JUAREZ_0318]]

## Chronology Cross-Reference

- [[CAP_HAMNETT_JUAREZ_0129|Chronology pp.252–253]] places the 1856-06-25 Ley Lerdo beside the Constituent Congress in the early Reform sequence.
- Planned `FACT_HAMNETT_JUAREZ_3157` was not created because this card, [[TIME_HAMNETT_JUAREZ_0318|0318]], and [[TIME_HAMNETT_JUAREZ_0416|0416]] preserve the two claims separately.

## Dramatis Personae Cross-Reference

- [[CAP_HAMNETT_JUAREZ_0139|p.273]] repeats Miguel Lerdo de Tejada's association with the Ley Lerdo and corporate-property policy.
- Planned `FACT_HAMNETT_JUAREZ_3405` was not created because this card and [[TIME_HAMNETT_JUAREZ_0318|0318]] already preserve the claim.
- The user-supplied candidate date 1856-06-26 differs from this card's 1856-06-25; the existing date remains unchanged pending verification.
